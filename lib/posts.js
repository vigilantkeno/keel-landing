...
export function validatePost(p) {
  ...
  // Illustration-system art (content/art/STYLE_SPEC.md): a declared hero must
  // actually exist under public/, and shipping an image without alt text is
  // an accessibility failure, not a style choice.
  if (p.heroImage) {
    if (!existsSync(join(process.cwd(), 'public', p.heroImage.replace(/^\//, '')))) {
      errs.push(`${where}: heroImage "${p.heroImage}" has no file under public/`);
    }
    if (!p.heroAlt || !String(p.heroAlt).trim()) {
      errs.push(`${where}: heroImage is set but heroAlt is missing — alt text is required`);
    }
  }
