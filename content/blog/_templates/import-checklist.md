...
7. **One CTA total** — packages sprinkle early-access references (product
   ...
   2026-08-01).
8. **Dates** — publish dates follow the editorial calendar cadence the
   founder sets, not the merge date.
9. **Verify rendered** — build, then check the post page for: metaTitle in
   <title>, description/canonical/OG/Twitter tags, Article + FAQPage +
   BreadcrumbList JSON-LD, breadcrumb trail, related-posts block.
10. **PR** — one post per PR; description records every deviation from the
   package (links repointed, claims rephrased, citations swapped) so the
   package author can improve the generator.
11. **Not actioned from packages**: social atoms (used manually after
